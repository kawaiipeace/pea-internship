...
interface DetailAttendanceBadgeProps {
    status: string;
    note?: string;
    leaveType?: string;
}

const DetailAttendanceBadge: React.FC<DetailAttendanceBadgeProps> = ({ status, note = '', leaveType }) => {
    if (status === 'LEAVE' || status === 'SICK' || status === 'ABSENCE') {
        const isSick = status === 'SICK' || 
                       leaveType === 'SICK' || 
                       note.includes('ป่วย') || 
                       note.includes('อาหารเป็นพิษ') || 
                       note.includes('แพทย์') || 
                       note.includes('โรงพยาบาล');
...
export default DetailAttendanceBadge;
