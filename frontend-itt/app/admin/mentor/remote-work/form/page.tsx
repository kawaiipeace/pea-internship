...
const RemoteWorkFormPage = () => {
  ...
  const [busyStudentsByDate, setBusyStudentsByDate] = useState<
    Record<string, { id: string; mentor: string }[]>
  >({});
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        const deptId =
          profileData.departmentId || profileData.profile?.departmentId;
        ...
        if (isEditMode) {
          const taskResponse = await axiosInstance.get(
            `/offsite-tasks/${taskId}`,
          );
          const task = taskResponse.data;

          // Format Date for HTML5 input (YYYY-MM-DD)
          if (task.workDate) {
            const formattedDate = new Date(task.workDate)
              .toISOString()
              .split("T")[0];
    ...
    const fetchBusyStudents = async () => {
      ...
      try {
        ...
        for (const monthKey of monthsToFetch) {
          ...
          const response = await axiosInstance.get("/offsite-tasks", {
            params: {
              viewMode: "all",
  ...
  const handleSubmit = async () => {
    ...
    if (workDates.length === 0)
      newErrors.workDate = "กรุณาระบุระยะเวลาปฏิบัติงาน";
    ...
    if (studentIds.length === 0)
      newErrors.studentIds = "กรุณาเลือกนักศึกษาอย่างน้อย 1 คน";
    ...
    const result = await Swal.fire({
      ...
      customClass: {
        ...
        confirmButton:
          "flex-1 h-[48px] bg-[#11A75C] hover:bg-[#0E8F4D] text-white rounded-[12px] text-[16px] font-bold order-2 shadow-md transition-colors",
        cancelButton:
    ...
    try {
      ...
      await Swal.fire({
        ...
        customClass: {
          popup:
            "rounded-[24px] p-10 w-auto min-w-[300px] bg-white dark:bg-[#1A1A1A] shadow-xl",
        },
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-black py-4 px-4">
      {/* Main Form Container */}
      <div className="mx-auto w-full max-w-[700px] bg-white dark:bg-[#121212] border border-[#CECFD2] dark:border-gray-700 rounded-[15px] shadow-sm flex flex-col p-6 sm:p-10 overflow-visible">
        {/* Header */}
        <div className="mb-8">
          ...
          <p className="text-[16px] text-[#61646C] dark:text-gray-400">
            {isEditMode
              ? "แก้ไขรายละเอียดกำหนดการปฏิบัติงานนอกสถานที่"
        ...
        <div className="flex-1 flex flex-col gap-4 overflow-visible">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-[50px] h-[50px] bg-[#FDF2FE] dark:bg-[#251025] border border-[#F9E1F9] dark:border-[#3d1a3d] rounded-full flex items-center justify-center shrink-0">
                <span className="material-symbols-rounded !text-[32px] text-[#A80689]">
                  edit_calendar
                </span>
              </div>
              <div>
                <h2 className="text-[20px] font-bold text-[#A80689]">
                  ระยะเวลาปฏิบัติงาน
                </h2>
                <p className="text-[14px] text-[#61646C] dark:text-gray-400 ">
            ...
            <div className="w-full sm:w-[348px]">
              <CustomDatePicker
                ...
                placeholder={
                  isEditMode ? "วว/ดด/ปปปป" : "เลือกวันที่ปฏิบัติงาน"
                }
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-[20px] font-bold text-[#A80689]">
                  สถานที่การปฏิบัติงาน
                </h2>
                <p className="text-[14px] text-[#61646C] dark:text-gray-400 ">
                  ระบุสถานที่การทำงานนอกสถานที่ให้ชัดเจน
                </p>
              </div>
            </div>
            <div>
              ...
              />
              {errors.locationName && (
                <p className="text-[#D92D20] text-[12px] mt-0.5">
                  {errors.locationName}
                </p>
              )}
            </div>
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  if (errors.taskDetail)
                    setErrors((prev) => ({ ...prev, taskDetail: "" }));
                }}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[14px] text-[#61646C] dark:text-gray-400 ">
                  เลือกรายชื่อนักศึกษาที่ทำงานนอกสถานที่
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
                ...
                return (
                  ...
                  >
                      ...
                      <span className="text-[14px] text-[#333] font-semibold truncate pr-4">
                        {student
                          ? (() => {
                              ...
                              const nickName =
                                student.nickname || student.displayUsername;
                              return nickName
                                ? `${fullName} (${nickName})`
                                : fullName;
                            })()
                    ...
                    {openDropdownIndex === index && (
                      <div className="absolute top-[50px] left-0 w-full max-w-[618px] max-h-[200px] overflow-y-auto bg-white dark:bg-[#1A1A1A] border border-[#A80689]/20 rounded-[10px] shadow-lg z-20 py-1 scrollbar-thin scrollbar-thumb-gray-300">
                        {availableStudents
                          .filter(
                            (s) => s.id === id || !studentIds.includes(s.id),
                          )
                          .map((s) => {
                            ...
                            return (
                              <div
                                key={s.id}
                                className={`px-4 py-2.5 mx-1 my-0.5 rounded-[8px] text-[14px] transition-all duration-200 ${isBusy ? "cursor-not-allowed bg-gray-50 text-gray-600" : s.id === id ? "bg-[#A80689] text-white cursor-pointer" : "text-[#101828] hover:bg-[#A80689] hover:text-white cursor-pointer"}`}
                                onClick={() =>
                                  !isBusy && handleSetStudent(index, s.id)
                                }
                              >
                                <div className="flex justify-between items-center w-full">
                                  ...
                                  {isBusy && (
                                    <span className="text-[10px] bg-red-100 text-red-800 border border-red-300 px-2 py-0.5 rounded-full ml-2 shrink-0 font-bold">
                                      ติดงานกับ {busyMentor}
                                    </span>
                                  )}
                                </div>
                              </div>
              ...
              {showEmptySlot && (
                <div
                  className="relative"
                  ref={(el) => {
                    dropdownRefs.current[studentIds.length] = el;
                  }}
                >
                  <div
                  ...
                  >
                    ...
                    <span
                      className={`material-symbols-rounded ${errors.studentIds ? "text-[#D92D20]" : "text-[#A80689]"} transition-transform duration-200 ${openDropdownIndex === studentIds.length ? "rotate-180" : ""}`}
                    >
                      expand_more
                    </span>
                  </div>
                  {errors.studentIds && (
                    <p className="text-[#D92D20] text-[12px] mt-0.5 ml-1">
                      {errors.studentIds}
                    </p>
                  )}
                        ...
                        availableStudents
                          ...
                          .map((s) => {
                            ...
                            return (
                              <div
                                key={s.id}
                                className={`px-4 py-2.5 mx-1 my-0.5 rounded-[8px] text-[14px] transition-all duration-200 ${isBusy ? "cursor-not-allowed bg-gray-100 text-gray-600" : "text-[#101828] hover:bg-[#FEEBFB] cursor-pointer"}`}
                                onClick={() =>
                                  !isBusy &&
                                  handleSetStudent(studentIds.length, s.id)
                                }
                              >
                                <div className="flex justify-between items-center w-full">
                                  <span>
                                  ...
                                  {isBusy && (
                                    <span className="text-[12px] bg-red-100 text-[#B42318] border border-red-300 px-2 py-0.5 rounded-full ml-2 shrink-0 ">
                                      ติดงานกับ {busyMentor}
                                    </span>
                                  )}
                                </div>
              ...
              {studentIds.length < availableStudents.length &&
                !showEmptySlot && (
                  <div
                  ...
                  >
                    <span className="material-symbols-rounded text-[#61646C]">
                      add
                    </span>
                    <span className="text-[#61646C] text-[16px]">
                      เพิ่มนักศึกษา
                    </span>
                  </div>
                )}
