...
function getBadge(app: AllStudentsHistoryItem, tab: ActiveTab) {
  ...
  if (tab === "approved") {
    if (app.studentInternshipStatus === "CANCEL") {
      return { text: "ยกเลิกฝึกงาน", className: "bg-[#FEE4E2] text-[#912018] border border-[#FECDCA]" };
    }
    if (app.studentInternshipStatus === "COMPLETE") {
      return { text: "ฝึกงานเสร็จสิ้น", className: "bg-[#DCFAE6] text-[#085D3A] border border-[#DCFAE6]" };
    }
...
function AdminApplicationsPage() {
  ...
  // Stats
  const stats = useMemo(() => {
    const pendingUpload = applications.filter((a) => getDocStatus(a) === "pending_upload").length;
    const pendingReview = applications.filter((a) => getDocStatus(a) === "pending_review").length;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <p className="text-gray-500 mb-6">ค้นหาใบสมัครสถานะ{pageTitle}ได้ {filtered.length} รายการ</p>

        {/* Stats Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          <div className={`bg-white rounded-xl border p-5 cursor-pointer transition-all ${activeTab === "pending_upload" ? "border-l-4 border-gray-600 ring-1 ring-gray-600" : "border-gray-300 hover:border-gray-600/60"}`} onClick={() => setActiveTab("pending_upload")}>
            <div className="flex items-center gap-3 mb-2">
              <div
                className={`w-5 h-5 ${activeTab === "pending_upload" ? "text-gray-600" : "text-primary-600"
                  }`}
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M5 15.5V18C5 18.5523 5.44772 19 6 19H18C18.5523 19 19 18.5523 19 18V15.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
            <p
              className={`text-2xl font-bold leading-tight ${activeTab === "pending_upload" ? "text-gray-600" : "text-primary-600"
                }`}
            >
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              <div
                className={`w-5 h-5 ${activeTab === "rejected" ? "text-[#F04438]" : "text-primary-600"
                  }`}
              >
                <svg
                  width="13"
            ...
            <p
              className={`text-2xl font-bold leading-tight ${activeTab === "rejected" ? "text-[#F04438]" : "text-primary-600"
                }`}
            >
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-primary-600 text-white">
                  <th className="px-4 py-3 text-left text-base font-semibold">ลำดับ</th>
                  <th className="px-4 py-3 text-left text-base font-semibold">ชื่อ-นามสกุล</th>
                  <th className="px-4 py-3 text-left text-base font-semibold">เบอร์โทรศัพท์</th>
                  {activeTab !== "rejected" && (
                    <th className="px-4 py-3 text-left text-base font-semibold">สถานะผู้สมัคร</th>
                  )}
                  <th className="px-4 py-3 text-left text-base font-semibold">ชื่อสถาบัน</th>
                  <th className="px-4 py-3 text-left text-base font-semibold">หน่วยงาน</th>
                  {activeTab === "rejected" && (
                    <th className="px-4 py-3 text-left text-base font-semibold">วันที่ตรวจ</th>
                  )}
                  <th className="px-4 py-3 text-left text-base font-semibold">ระยะเวลาฝึกงาน</th>
                </tr>
              </thead>
              <tbody>
                {paginated.length === 0 ? (
                  ...
                  const badge = activeTab !== "rejected" ? getBadge(app, activeTab) : null;
                  return (
                    <tr
                      key={app.applicationId}
                      className="border-b border-gray-200 hover:bg-gray-50 transition-colors cursor-pointer"
                      onClick={() => router.push(`/admin/dashboard/${app.applicationId}`)}
                    >
